...
To take a second sample of size 50, we can use the sample_n()
...
We can calculate the sample proportion for samp2 as we did for samp1:
...
ggplot() + 
  ...
  labs(x = "Belief about whether scientific research benefits people",
       y = "Count",
       title = "Comparison of Sample Proportions")
...
There are 15,000 elements in sample_props50, which is the result of taking 15,000 different samples of size 50 from the population and calculating the proportion of "Doesn't benefit" responses in each sample.
...
```{r}
center <- mean(sample_props50$p_hat)
```
...
hist(sample_props15, breaks = sqrt(length(sample_props15)), 
     main = "Sampling Distribution of Proportion of Those Who Think Scientists Enhance Lives (n=15)")

```

To calculate and report the population proportion, we can use the table function to summary of the scientist_work variable
...
hist(sample_props150, breaks = sqrt(length(sample_props150)), 
     main = "Sampling Distribution of Proportion of Those Who Think Scientists Enhance Lives (n=150)")

```
    

Based on this sampling distribution, I would guess that the true proportion of those who think the work scientists do enhances their lives is around 0.8, which is consistent with the population proportion we calculated earlier.
